...
process_app() {
  ...
  if [[ ! -d "$clone_dir" ]]; then
    ...
    git clone --depth 1 --branch "$ref" "$repo" "$clone_dir" 2>/dev/null || \
      git clone --depth 1 "$repo" "$clone_dir" && cd "$clone_dir" && git checkout "$ref" 2>/dev/null; cd - >/dev/null
  fi
...
# Iterate over apps from config
apps=$(
  if [[ -n "$APP_FILTER" ]]; then
    yq -e ".apps | to_entries | map(select(.key == \"$APP_FILTER\")) | from_entries" "$CONFIG" 2>/dev/null || {
