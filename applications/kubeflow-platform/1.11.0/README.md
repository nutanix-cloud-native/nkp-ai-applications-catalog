...
reuses NKP Dex for identity and provisions a dedicated Kubeflow LoadBalancer in
front of `istio-helm` ingressgateway pods. The core pieces are
...
| Ingress URL / host | Read from the dedicated `kubeflow-ingressgateway` LoadBalancer | `config.kubeflowIngressURL`, `config.kubeflowIngressHost` |
...
If istio-helm isn't Ready yet, the app retries until the dedicated ingress
Service receives an external address - no action needed. To override anything,
set it under `config` in the app's configuration (Kommander UI → the app →
Configuration, or an `AppDeployment`'s `configOverrides`).
